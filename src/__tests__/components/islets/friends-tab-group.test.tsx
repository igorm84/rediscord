...
import Meta, { Template } from "@/components/islets/friends-tab-group/friends-tab-group.stories";
...
describe('FriendsTabGroup', () => {
    it('tab group should render proper count of tabs', () => {
        ...
        expect(element.getByTestId("tab-group").childNodes.length)
            .toBe(Object.values(friendsTabsProps).length);
    })
    it("tab group should correct render active tab", async () => {
        const element = render(<FriendsTabGroup />);
        const tabs = element.getByTestId("tab-group").childNodes as unknown as Element[];
        // pick random tab
